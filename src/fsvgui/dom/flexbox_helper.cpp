...
void SetX(Global& global, std::vector<Line> lines)
{
    for (auto& line : lines) {
        ...
        for (auto* block : line.blocks) {
            ...
            element.flex_grow   = block->flex_grow_x != 0 || global.config.justify_content == FlexboxConfig::JustifyContent::Stretch
                  ? 1
                  : 0;
...
void SetY(Global& g, std::vector<Line> lines)
{
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        for (auto* block : lines[i].blocks) {
            const bool stretch = block->flex_grow_y != 0 || g.config.align_content == FlexboxConfig::AlignContent::Stretch;
            const int size     = stretch ? element.size : std::min(element.size, block->min_size_y);
...
{
    if (global.config.direction == FlexboxConfig::Direction::Column || global.config.direction == FlexboxConfig::Direction::ColumnInversed) {
        SymmetryXY(global);
        Compute1(global);
        SymmetryXY(global);
        return;
